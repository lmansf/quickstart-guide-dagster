...
The long versions of everything abridged at the bottom of the [README](../README.md).

> **First, the classic:** if `all_promo_orders_attributed` is **red** on your first
> materialization — that is not a bug, that is the tutorial. See README **Step 5**. Every
...
(If you have Git Bash or WSL, the macOS/Linux commands in the README work as-is; `winget
...
- **Does not** touch run history or sensor state. Both live in `.dagster_home/` — and both
  ...
  (Step 3), then toggle the sensor and `make new-day`.
...
in `cadence/data_gen.py`, so the Step 5 story survives a re-seed.

## uv behind a corporate proxy

**Symptom** — the `curl … astral.sh` / `irm … astral.sh` installers from README Step 0 are
...
Remember: the network is needed **only** for Step 0 (installing uv) and Step 1's first
